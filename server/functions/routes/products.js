...
// get all the cartitems for that users
  ...
  (req, res) => {
    ...
    if (endpointSecret) {
      ...
      try {
        ...
        res.status(400).send(`Webhook Error:${err.measage}`);
        return;
      }
    ...
    // Return a 200 res to acknowledge reciept the event
    res.send().end();
  }
);
...
const deleteCart = async (userId, items) => {
  ...
  items.map(async (data) => {
    ...
    await db
      ...
      .doc(`/${data.productId}/`)
      .delete()
      .then(() => console.Console.log("--------------successs---------------"));
  });
};

// Fetching Orders from collection and push it to redux store
